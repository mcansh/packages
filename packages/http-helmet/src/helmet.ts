...
type BaseSecureHeaders = {
  ...
  /**
   * The HTTP Feature-Policy header provides a mechanism to allow and deny the
   * use of browser features in its own frame, and in content within any
   ...
   * (sent with the Referer header) should be included with requests. Aside from
   * the HTTP header, you can set this policy in HTML.
   *
  ...
  /**
   * HTTP Strict-Transport-Security response header (often abbreviated as HSTS)
   ...
   * feature by which browsers proactively perform domain name resolution on
   * both links that the user may choose to follow as well as URLs for items
   * referenced by the document, including images, CSS, JavaScript, and so
   * forth.
   *
   ...
   * reflected cross-site scripting (XSS) attacks. These protections are largely
   ...
   * origins (such as those issued with elements like <script> and <img>), to
   * mitigate speculative side-channel attacks, like Spectre, as well as
...
export type CreateSecureHeaders = RequireOneOrNone<
  {
     ...
     * their effects. These violation reports consist of JSON documents sent via
     * an HTTP POST request to the specified URI defined in a
     * Reporting-Endpoints HTTP response header.
     *
